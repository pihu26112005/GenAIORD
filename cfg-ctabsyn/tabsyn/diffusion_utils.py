...
#####
def sample(net, num_samples, dim, label, num_steps = 50, device = 'cuda:0'):
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            x_next = sample_step(net, num_steps, i, t_cur, t_next, x_next, label)
            #####

    return x_next

def sample_step(net, num_steps, i, t_cur, t_next, x_next, label):
